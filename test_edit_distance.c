...
void test_pattern_based() {
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        printf("  Identical[%6zu]: %d (expected %d) %s\n", 
               len, result, expected, (result == expected) ? "PASSED" : "FAILED");
        ...
        printf("  AllDiff[%6zu]:   %d (expected %zu) %s\n", 
               len, result, len, (result == expected) ? "PASSED" : "FAILED");
        assert(result == expected);
...
void test_edge_cases() {
    ...
    printf("  All same (%zu chars): %d (expected 0) %s\n", 
           len, result, (result == 0) ? "PASSED" : "FAILED");
    assert(result == 0);
    free(same1);
    free(same2);
...
int main() {
    ...
    printf("========================================\n");
    printf("ALL TESTS PASSED! PASSED\n");
    printf("========================================\n");
